...
class NegotiationGRPOTrainer:
        ...
        for group in groups:
            ...
            for traj in group.trajectories:
                ...
                for turn in traj.active_turns:
                    # 截断过长 prompt
                    p_ids = turn.prompt_token_ids[-cfg.max_prompt_length:]
    ...
    def _grpo_step(
        ...
        # 用当前 policy 重新算 logp（带 grad）
        new_logps = self._compute_logps_with_grad(samples, use_adapter=active_adp)
        ...
        for s, lp_new, lp_old, lp_ref in zip(samples, new_logps, old_logps, ref_logps):
            adv = s.advantage
    ...
    def _log_step(
        ...
        deal_rate = sum(1 for t in all_trajs if t.final_state.outcome.is_deal) / n
        avg_reward = sum(t.advantage_reward for t in all_trajs) / n
        avg_rounds = sum(len(t.final_state.history) for t in all_trajs) / n / 2  # 一轮=2 turn
        ...
        if self.global_step % self.config.logging_every == 0:
            self.logger.log(
                ...
                f"deal_rate={deal_rate:.2%} reward={avg_reward:.2f} rounds={avg_rounds:.1f} "
